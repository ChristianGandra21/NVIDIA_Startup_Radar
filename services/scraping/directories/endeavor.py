"""
Scraper for Endeavor Brasil (endeavor.org.br) – supports scale-up startups.

Extracts startup profiles from Endeavor articles using trafilatura + LLM.
"""
...
INDEX_URL = "https://endeavor.org.br/ambiente/"
EXTRACTION_METHOD = "trafilatura+llm_endeavor"
...
def list_article_urls(index_html: str) -> list[str]:
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if href.startswith("/ambiente/") and href != "/ambiente/":
            urls.add(f"https://endeavor.org.br{href}")
    return sorted(urls)
